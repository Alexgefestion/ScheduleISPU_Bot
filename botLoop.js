...
const VkToken = 'vk1.a.3qRx5cRLpl7Qsu-PA2WBJ6gmn2MJrIoWI2OgYuuJEqOaYN8UtKAt7ZBpjml5WkVDHjkeLvtd2blU_hjnLS9myxSt3h0643w3opOF9lmtMQM_JcZ8iOQePok6nn6IMQzPyBNgqY8S9KTtLlH-5zrrWe140A7Gt6Lv7yb92sD2gJt6eKgPl2mLEouxGU-nKFj8ajah6ZCiKuhbxRNkmVVBSw';
const vkBot = new VkBot(VkToken);
...
let VkPerson = JSON.parse(fs.readFileSync('personVk.json', { encoding: 'UTF8', flag: 'r+' }));
...
function createKeyboardFS(path, arg1, arg2, arg3) {
  ...
  fs.readdir(`schedule/постоянное/${path}`, (err, list) => {
    ...
    for(let i = 0; i < Math.ceil(list.length/5); i++){
      ...
      for(let j = 0; j < 5 && i*5+j < list.length; j++){

        if(list[i*5+j][list[i*5+j].length-4] != '.'){
          Keyboard.buttons[i][j] = {action: {type: "callback", payload: {"choose": arg3, "data": path + list[i*5+j]}, label: list[i*5+j]}};
        }else{
          Keyboard.buttons[i][j] = {action: {type: "callback", payload: {"choose": arg3, "data": path + list[i*5+j]}, label: list[i*5+j].slice(0, list[i*5+j].length-4)}};
        }

      }
    }
    vkBot.sendMessage(arg1, arg2, null, Keyboard);
  ...
  for(let i = 0; i < VkPerson.length; i++){
    if(VkPerson[i][0] == chatId){
      ...
      if(weekNumArg == 0){
        ...
        if(weekNum == 1){
          compliteShedule1 = ' (Текущая) \n\r\n\r';
        }else{
          compliteShedule2 = ' (Текущая) \n\r\n\r';
        }
        for(let i = 0; i<7; i++){
        ...
        compliteData = dayOnWeek[dayNumArg]+', '+weekEvenOdd[0]+compliteShedule1+'\n\r\n\r'+dayOnWeek[dayNumArg]+', '+weekEvenOdd[1]+compliteShedule2;
        ...
        compliteData = dayOnWeek[dayNumArg]+', '+weekEvenOdd[weekNumArg-1]+' '+TextArg+'\n\r\n\r'+compliteShedule;
      }
      vkBot.sendMessage(chatId, compliteData);
      return;
    }
  }
}
...
vkBot.event('message_new', async (ctx) => {
  console.log(ctx)
  try {
    if(parseInt(ctx.bot.longPollParams.ts) < parseInt(ctx.bot.ts)) {
...
vkBot.event('message_event', async (ctx) => {
  ...
  try {
    if(parseInt(ctx.bot.longPollParams.ts) < parseInt(ctx.bot.ts)) {
      ...
      let data = ctx.message.payload.data;
      let eventId = ctx.message.event_id;
      let chatId = ctx.message.user_id;

      if(payload.choose === 'факультет'){
        await createKeyboardFS(data+'/', chatId, 'Выберите курс', 'курс');
        return vkBot.eventAnswer(eventId, chatId, chatId);
        // return vkBot.deleteMessage(ctx.message.groupId, ctx.message.conversation_message_id, 1, 0, ctx.message.peer_id);
      }

      if(payload.choose === 'курс'){
        await createKeyboardFS(data+'/', chatId, 'Выберите группу', 'группа');
        return vkBot.eventAnswer(eventId, chatId, chatId);
        // return vkBot.deleteMessage(ctx.message.groupId, ctx.message.conversation_message_id, 1, 0, ctx.message.peer_id);
      }

      if(payload.choose === 'группа'){
        vkBot.sendMessage(chatId,  'Ваша группа: '+ data.slice(0,data.length-4), null, {
          "buttons": [
            [
              {action: {type: "text", payload: {}, label: "сегодня"}},
              {action: {type: "text", payload: {}, label: "завтра"}}
            ],
            [
              {action: {type: "text", payload: {}, label: "пн"}},
              {action: {type: "text", payload: {}, label: "вт"}},
              {action: {type: "text", payload: {}, label: "ср"}}
            ],
            [
              {action: {type: "text", payload: {}, label: "чт"}},
              {action: {type: "text", payload: {}, label: "пт"}},
              {action: {type: "text", payload: {}, label: "сб"}}
            ]
          ]
        });
        vkBot.eventAnswer(eventId, chatId, chatId);
        for(let i = 0; i < VkPerson.length; i++){
          if(VkPerson[i][0] == chatId){
            VkPerson[i][1] = data;
            break;
          }else{
            if(i === VkPerson.length-1){
              VkPerson.push([chatId, data, 1]);
            }
          }
        }
        fs.writeFileSync('personVk.json', JSON.stringify(VkPerson, null, 4));
        return;
      }
...
// 	{command: '/help', description: 'инструкция пользователя'},
// ]);
